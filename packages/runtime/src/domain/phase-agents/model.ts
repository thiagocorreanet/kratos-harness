...
const sharedInstructions = [
  ...
  "",
  "Set `contractVersion` and `hostContract` to `1.1.0`. The object also carries",
  "`agent`, `outcome`, `artifacts`, `changedFiles`, and `payload`. Keep artifact",
  "Set `contractVersion` and `hostContract` to `1.2.0`. The object also carries",
  "`agent`, `outcome`, `artifacts`, `changedFiles`, `payload`, and `memory`. Keep artifact",
  "documents separate from source and test changes. Use project-relative paths.",
  "For code or review, read the confirmed-lessons section of",
  "`.brain/03-memory/gotchas.md` before work and return the exact `memory`",
  "observation supplied by the runtime handoff. For every other phase, return",
  "`memory: null`. This is an acknowledgement, never a workflow decision.",
  "",
...
const sources = [
  {
    ...
    role: [
      ...
      "and the confirmed-lessons section of `.brain/03-memory/gotchas.md`",
  ...
  },
  {
    ...
    role: [
      ...
      "step, the confirmed-lessons section of `.brain/03-memory/gotchas.md`, the implementation diff, mapped tests, command results, and recorded",
      ...
      "The handoff carries ordered attempt context and `faultsRequiredFor`.",
      "The runtime decides which fault reports are required; relay that list",
      "without recalculating it. For each named criterion that fails, report one",
      "fault with `criterionId`, `classification` (`code` or `specification`),",
      "and a concise diagnosis. Do not report faults for other criteria.",
    ],
    payload: [
      ...
      "exactly `verdict`, `criteria`, and optional `faults`; each criterion carries",
      "`criterionId`, `outcome`, and `evidenceRef`. Each fault carries the fields",
      "named by the handoff instructions. Keep `artifacts` and `changedFiles` empty.",
    ],
  },
  {
    id: "prd-researcher",
