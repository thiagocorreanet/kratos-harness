...
/**
 * What this runtime refuses today, recorded where a person can read it.
 *
 * The workflow gates ported from the Go implementation belong to `SDD-04`;
 * claiming them here would describe rules nothing enforces yet.
 */
...
function stackProfileDocument(profile: StackProfile): string {
  const preamble = [
    "# Stack profile",
    "",
    "Written by Mestre Yoda from the names of the entries at the project root.",
...
function codexConfiguration(answers: Answers): string {
  return lines(
    `# Managed by Mestre Yoda ${YODA_VERSION}.`,
    "",
...
function instructions(answers: Answers, file: string, note: string): string {
  return lines(
    ...
    "# Mestre Yoda",
    "",
    `Mestre Yoda ${YODA_VERSION} manages this section of ${file}. Anything`,
