...
| Pre-profile configurations require explicit migration and preserve existing state | `tests/config-migration.test.ts` covers the ordered `1.0.0` → `1.1.0` → `1.2.0` → `1.3.0` chain, persisted roles/language, migration answers, unresolved new leaves, exact plans, rollback, and `profile.config_migration_required`. |
...
| Doctor warns actionably for unresolved keys, a missing document, or byte drift | `tests/diagnostics.test.ts` pins warning details. `tests/doctor-command.test.ts` requires typed key paths and regeneration guidance from the real command without parsing Markdown. |
...
| Release packages enforce relay behavior rather than only asset text | `tests/package-verifier.test.ts` recomputes a tampered host-assets digest and still requires verification to reject the changed questions/value mapping. `scripts/verify-package.mjs` imports and executes each packaged relay. |
