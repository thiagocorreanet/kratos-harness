...
| **AC-8**: Gates requiring operator consent fail closed on unconfirmed `derived` values with diagnostic reason | `tests/diagnostics.test.ts` and `tests/doctor-command.test.ts` verify that unconfirmed derived profile leaves remain distinct from operator-confirmed `resolved` leaves, ensuring operator decision gates require explicit human approval and doctor diagnostic reporting enforces clear state boundaries. |
