...
Kratos uses one pinned development environment to make a clean checkout
...
| `npm run english:check` | Reject non-English repository prose outside declared historical artifacts |
...
| `npm run benchmark` | Measure built help, version, and handshake p95 against published budgets |
...
The build stages two independent host packages in
`${KRATOS_BUILD_OUTPUT}` or the operating system temporary directory:

```text
kratos-plugin-build/codex/
kratos-plugin-build/claude-code/
```

Each package contains an executable `runtime/kratos.mjs`, its modular embedded
runtime, contracts, schemas, integrity manifest, and only that host's thin
adapter assets. The checkout never receives a generated `dist` directory.

Package verification checks both inventories, forbidden files, runtime and host
digests, manifest coherence, and plugin-relative runtime paths. It then installs
each host package into a clean room, executes version and handshake, initializes
a Git project, and exercises objective, start, and status. Verification rejects
any initialized project that receives motor files or dependencies.
