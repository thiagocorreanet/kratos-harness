...
describe("contract compatibility classifier", () => {
  it("recognizes stateContract 1.3.0 and hostContract 1.3.0 as current", () => {
    expect(CONTRACT_IDENTITIES.state).toBe("1.3.0");
    expect(CONTRACT_IDENTITIES.host).toBe("1.3.0");
    expect(CONTRACT_VERSIONS["state.project-config"]).toBe("1.3.0");
    expect(CONTRACT_VERSIONS["host.init-answers"]).toBe("1.3.0");
  });
