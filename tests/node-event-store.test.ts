...
async function temporaryProject<T>(
  ...
  const root = await mkdtemp(join(tmpdir(), "kratos-node-event-store-"));
  const outside = await mkdtemp(
    join(tmpdir(), "kratos-node-event-store-outside-"),
  );
  try {
...
function countingDurableFileSystem(root: string): {
  ...
  return {
    ...
    durableFileSystem: {
      ...
      renameDirectoryExclusive: async (sourcePath, targetPath) => {
        calls.push("rename_directory_exclusive");
        return actual.renameDirectoryExclusive(sourcePath, targetPath);
      },
