...
import { nodeDurableFileSystem } from "@kratos/runtime/infra/node";
...
async function temporaryProject<T>(
  ...
  const root = await mkdtemp(join(tmpdir(), "kratos-node-security-"));
  const outside = await mkdtemp(join(tmpdir(), "kratos-node-outside-"));
  try {
...
describe("node durable filesystem security", () => {
  ...
  it.each([
    ...
    async (cutPoint) => {
      const container = await mkdtemp(
        join(tmpdir(), "kratos-node-capture-swap-"),
      );
  ...
  it.each(["ENOENT", "EACCES"])(
    ...
    async (code) => {
      const root = join(tmpdir(), `kratos-node-root-${code.toLowerCase()}`);
  ...
  it("rejects a root symlink swap before the first durable operation", async () => {
    const container = await mkdtemp(join(tmpdir(), "kratos-node-root-swap-"));
    const root = join(container, "project");
    const displaced = join(container, "project-original");
    const outside = join(container, "outside");
    try {
