...
import type { DurableOperation } from "@kratos/runtime/infra/node";
...
async function temporaryProject<T>(
  ...
  const root = await mkdtemp(join(tmpdir(), "kratos-process-recovery-"));
  try {
...
beforeAll(async () => {
  bundleRoot = await mkdtemp(join(tmpdir(), "kratos-process-worker-"));
