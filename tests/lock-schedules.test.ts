...
import type { LeaseGuard } from "@kratos/runtime/domain/locks";
 ...
 * `KRATOS_TEST_EXHAUSTIVE_LOCK_SCHEDULES=1` for the same reason the event-chain
...
const schedules = permutations(ACTIONS).filter(
  (schedule) =>
    process.env.KRATOS_TEST_EXHAUSTIVE_LOCK_SCHEDULES === "1" ||
