...
describe("the generated skeleton", () => {
  ...
  it("says plainly when no known stack matched", () => {
    ...
    expect(profile).toContain("No known toolchain marker was found");
    expect(profile).toContain("The technology was not identified.");
  });
  ...
  it("enforces strict budget on the managed instructions block (<= 50 lines and <= 2000 chars)", () => {
    ...
    for (const file of ["CLAUDE.md", "AGENTS.md"]) {
      ...
      expect(content).toContain("Detected toolchains: node");
    }
  });
});
