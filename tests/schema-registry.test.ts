...
const bundledRuntime = runtimeEntry();
...
describe("Ajv schema registry", () => {
  ...
  it("fails closed on Object.prototype pollution present before module initialization", async () => {
    ...
    try {
      vi.resetModules();
      try {
        await import("@kratos/runtime/infra/schema");
      } catch (error) {
        importError = error;
      }
      importCalls = calls;
...
describe("schema registry fresh-process preflight", () => {
  ...
  it("rejects a throwing allowlisted accessor before any hostile execution", () => {
    const result = runWithPrototypePollution(`
      let calls = 0;
      process.on("exit", () => {
        process.stderr.write("\\nMESTRE_KRATOS_PROBE_CALLS=" + calls + "\\n");
      });
    ...
    expect(result.stderr).toContain("MESTRE_KRATOS_PROBE_CALLS=0");
  ...
  it("rejects a hidden Array.isArray accessor before initialization with zero host execution", () => {
    const result = runWithPrototypePollution(`
      ...
      process.on("exit", () => {
        process.stderr.write(
          "\\nMESTRE_KRATOS_ACCESSOR_CALLS=" + accessorCalls +
          "\\nMESTRE_KRATOS_REFLECTION_CALLS=" + reflectionCalls + "\\n",
        );
      });
      Object.defineProperty(Array, "isArray", {
        configurable: true,
    ...
    expect(result.stderr).toContain("MESTRE_KRATOS_ACCESSOR_CALLS=0");
    expect(result.stderr).toContain("MESTRE_KRATOS_REFLECTION_CALLS=0");
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("Embedded schema registry is invalid");
  });
});
